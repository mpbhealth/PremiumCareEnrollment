...
**PremiumCare Enrollment (this repo):** `src/utils/dependentEmailValidation.ts`, `src/components/DependentsAddressSection.tsx` (blur + local errors), `src/components/EnrollmentWizard.tsx` (submit validation).

**Premium HSA:** same file roles; paths mirror this layout.
...
## 8. Related files (PremiumCare Enrollment)

| File | Role |
|------|------|
| `src/utils/dependentEmailValidation.ts` | `DEPENDENT_EMAIL_DUPLICATE_ERROR` + `getDependentEmailDuplicateError` |
| `src/components/DependentsAddressSection.tsx` | `handleEmailBlur` (`currentTarget.value` + `dependents` snapshot), `localErrors`, merge `{ ...externalErrors, ...localErrors }` |
| `src/components/EnrollmentWizard.tsx` | `validateStep3` dependent email checks after format, using the same helper |
