...
| Location | Role |
|----------|------|
| `src/utils/dateCalculations.ts` | `calculateEffectiveDates()` — Eastern cutoff + three first-of-month options; `formatEffectiveDateDisplay` / `formatEffectiveDateLongDisplay` + `effectiveDateStringToUtcMs` for UTC-stable labels |
| `src/hooks/useEnrollmentStorage.ts` | Default **`effectiveDate`** = first returned option |
| `src/components/Step2AddressInfo.tsx` | Renders dropdown from `calculateEffectiveDates()`; summary uses `formatEffectiveDateDisplay` |
| `src/utils/generateEnrollmentPDF.ts` | PDF member block uses `formatEffectiveDateLongDisplay` for the chosen date |
