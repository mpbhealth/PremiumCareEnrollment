...
**Premium Care Enrollment (this repo):** [`src/utils/dependentPhoneSsnDuplicateValidation.ts`](src/utils/dependentPhoneSsnDuplicateValidation.ts), [`src/components/DependentsAddressSection.tsx`](src/components/DependentsAddressSection.tsx) (blur + `localErrors` + subscriber `useEffect`), [`src/components/Step2AddressInfo.tsx`](src/components/Step2AddressInfo.tsx) (subscriber fields + passing `phone`/`ssn` into dependent section props), [`src/components/EnrollmentWizard.tsx`](src/components/EnrollmentWizard.tsx) (`validateStep3`).

**Reference / sister pattern:** "Direct Enrollment" repos may use the same filenames.
