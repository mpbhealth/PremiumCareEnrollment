...
function extractSensitiveFields(payload: Record<string, unknown>): SensitiveFields {
  ...
  return {
    ...
    benefitId: (payload.benefitId as string | null) ?? null,
    pdid: (payload.pdid as number) || 0,
    phone: (payload.phone as string) || '',
    email: (payload.email as string) || '',
    agent: (payload.agent as string) || '',
    dob: (payload.dob as string) || '',
    city: (payload.city as string) || '',
    appliedPromo: (payload.appliedPromo as Record<string, unknown>) || null,
    payment: {
...
export async function encryptSensitiveFields(
  ...
  const sensitive = extractSensitiveFields(payload);
  const sensitiveJson = JSON.stringify(sensitive);
  const sensitiveBytes = new TextEncoder().encode(sensitiveJson);
  ...
  strippedPayload._encrypted = encryptedBlock;
